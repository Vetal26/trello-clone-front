...
export class BoardComponent implements OnInit {
  ...
  getArchiveTasks() {
    const idx = this.board.TaskLists.findIndex(
      (list) => list.name === 'Archive'
    );
    if (idx !== -1) {
  ...
  addTaskInArchive(task: Task) {
    ...
    this.taskService
      .updateTask(task.id, { task, activity })
      .subscribe((task) => {
  ...
  sendToBoard(task: Task) {
    ...
    this.taskService
      .restoreTask(task.id, { task, activity })
      .subscribe((res) => {
        const idx = this.board.TaskLists.findIndex(
          (taskList) => taskList.id === res.taskListId
        );
        this.board.TaskLists[idx].Tasks.push(res);
        const idxTask = this.archiveTasks.Tasks.findIndex(
          (task: any) => task.id === res.id
        );
